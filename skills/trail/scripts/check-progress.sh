#!/usr/bin/env bash
# Stop hook: show progress before agent stops and remind to update checkboxes.
...
# If trail binary is available, use it
if command -v trail &>/dev/null; then
  output="$(trail status 2>/dev/null)" || output=""
  if [ -n "$output" ]; then
    echo "[trail] $output"
    echo "[trail] Update checkboxes in $plan_file before stopping."
    exit 0
  fi
fi
...
if [ "$total" -gt 0 ]; then
  echo "[trail] Progress: $done/$total tasks done."
else
  echo "[trail] Active plan: $plan_name (no tasks defined yet)."
fi
echo "[trail] Update checkboxes in $plan_file before stopping."
